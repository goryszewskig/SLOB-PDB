...
function f_create_bfile_dir(){  # Note, as of SLOB 2.4 this function is on ice
#global OS_TEMP
#global SLOB_TEMPDIR
local constring="$*"
local ret=0
...
if [[ ! -d "$OS_TEMP" || ! -w "$OS_TEMP" ]]
then
	f_msg FATAL "${FUNCNAME}: The \$OS_TEMP environment variable is set to invalid directory (\"${OS_TEMP}\")."
	return 1
fi

f_msg NOTIFY "Creating (or replacing) SLOB BFILE (SLOBDIR) directory under ${OS_TEMP}."

sqlplus -L "$constring" <<EOF > $tmpfile 2>&1
WHENEVER SQLERROR EXIT 2;
CREATE OR REPLACE DIRECTORY slobdir AS '${OS_TEMP}';
EXIT;
EOF
...
if [ "$ret" -ne 0 ]
then
	f_msg FATAL "${FUNCNAME}: Failed to create (or replace) BFILE directory (SLOBDIR) under ${OS_TEMP}."
	cat $tmpfile
fi
...
then
	...
	then
		...
		do_rotor=TRUE	
		(( sqlnet_batch = $SCHEMAS / $SQLNET_SERVICE_MAX ))
...
do
	...
	slobargs="$arg1 $arg2 $arg3 $arg4 $arg5 $arg6 $arg7 $arg8 $arg9 $arg10 $arg11 $arg12 $arg13 $arg14 $arg15 $arg16 "
...
before=$SECONDS
f_msg NOTIFY "Test has been triggered. Processes are executing." >&2
